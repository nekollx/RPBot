...
exports.run = (client, msg) => {
	...
	var   playername  = msg.author.username;
        ...
        const RiderEmbed = new discord.RichEmbed()
	      ...
	      RiderEmbed.setThumbnail('http://www.maskedriders.info/Mee6RP/statusPic/Magimuscles.PNG')
              ...
              RiderEmbed.setFooter('Data Card ~' + settings.PlayerClassName(validform), settings.serverpic)
                      ...
                      if ((validform >= 5000) && (validform < 6000)){
                            if ((validform == 5100) || (validform == 5120) || (validform == 5110)){//wiseman, sorcerer, mage
                                    RiderEmbed.addField('Muscles', 'Now!', true)
                                    RiderEmbed.addField('Transform', 'Temporarily enhances Wizard\'s muscles to spectacular results. If in Rider form, wizard becomes a hulking giant that can easily destroy Ghouls.', true)
                            } else if (validform == 5000){//wizard
                                    RiderEmbed.addField('Excite', 'Please.', true)
                                    RiderEmbed.addField('Transform', 'Temporarily enhances Wizard\'s muscles to spectacular results. If in Rider form, wizard becomes a hulking giant that can easily destroy Ghouls.', true)
                            } else if (validform == 5140){//beast in magic land
                                    RiderEmbed.addField('Excite', 'Go!', true)
                                    RiderEmbed.addField('Transform', 'Temporarily enhances Wizard\'s muscles to spectacular results. If in Rider form, wizard becomes a hulking giant that can easily destroy Ghouls.', true)
