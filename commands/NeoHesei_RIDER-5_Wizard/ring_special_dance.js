...
exports.run = (client, msg) => {
	...
	var   playername  = msg.author.username;
        ...
        const RiderEmbed = new discord.RichEmbed()
	      ...
	      RiderEmbed.setThumbnail('http://www.maskedriders.info/Mee6RP/statusPic/Dance_Ring.png')
              ...
              RiderEmbed.setFooter('Data Card ~' + settings.PlayerClassName(validform), settings.serverpic)
                      ...
                      if ((validform >= 5000) && (validform < 6000)){
                            if ((validform == 5100) || (validform == 5120) || (validform == 5110)){//wiseman, sorcerer, mage
                                    RiderEmbed.addField('Party', 'Now!', true)
                                    RiderEmbed.addField('Special', 'Allows Wizard and Beast to perform a dance. The dance can also be used in combat. When used, it will use Life is SHOW TIME as its theme.', true)
                            } else if (validform == 5000){//wizard
                                    RiderEmbed.addField('Dance', 'Please.', true)
                                    RiderEmbed.addField('Special', 'Allows Wizard and Beast to perform a dance. The dance can also be used in combat. When used, it will use Life is SHOW TIME as its theme.', true)
                            } else if (validform == 5140){//beast in magic land
                                    RiderEmbed.addField('Dance', 'Go!', true)
                                    RiderEmbed.addField('Special', 'Allows Wizard and Beast to perform a dance. The dance can also be used in combat. When used, it will use Life is SHOW TIME as its theme.', true)
