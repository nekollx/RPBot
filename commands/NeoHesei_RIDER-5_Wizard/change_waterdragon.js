...
exports.run = (client, msg) => {
	...
	var   playername  = msg.author.username;
        ...
        const RiderEmbed = new discord.RichEmbed()
	      ...
	      RiderEmbed.setThumbnail('http://www.maskedriders.info/Mee6RP/statusPic/WaterDragonRing.PNG')
              ...
              RiderEmbed.setFooter('Data Card ~' + settings.PlayerClassName(validform), settings.serverpic)
                ...
                if ((validform >= 5000) && (validform < 6000)){
                            if (validform == 5100){//wiseman
                                    ...
                                    connection.query("UPDATE " + settings.dbtable + " SET CLASS_ID_Sub = 5221 WHERE ID = '" + playeruid + "'");
                                    ...
                                    connection.query("UPDATE " + settings.dbtable + " SET CLASS_ID_Sub = 5380 WHERE ID = '" + playeruid + "'");
                                    connection.query("UPDATE " + settings.dbtable + " SET Power_Mult = 5 WHERE ID = '" + playeruid + "'");
                            } else {//wizard
                                    RiderEmbed.setDescription('Level: 4', true)
                                    connection.query("UPDATE " + settings.dbtable + " SET CLASS_ID_Sub = 5021 WHERE ID = '" + playeruid + "'");
                                    connection.query("UPDATE " + settings.dbtable + " SET Power_Mult = 5 WHERE ID = '" + playeruid + "'");
                            }//if else

                                    if (validform == 5000){
                                        RiderEmbed.addField('Water, Dragon', 'Jabajaba Bashān, Zabun Zabun!', true)
                                        RiderEmbed.addField('Transform', 'Transforms into Water Dragon Style.', true)
                                        RiderEmbed.setImage('http://www.maskedriders.info/Mee6RP/statusPic/WaterDragonComboStatesStyle.png')
                                    } else if (validform == 5140){//beast in magic land
                                        RiderEmbed.addField('Water', 'Go!', true)
                                        RiderEmbed.addField('Transform', 'Transforms into Water Griphon Style.', true)
                                    } else { 
                                        RiderEmbed.addField('Hydro, Phantom', 'Jabajaba Bashān, Zabun Zabun!', true)
                                        RiderEmbed.addField('Transform', 'Transforms into Hydro Phantom Style.', true)
