...
exports.run = (client, msg) => {
	...
	var   playername  = msg.author.username;
        ...
        const RiderEmbed = new discord.RichEmbed()
	      ...
	      RiderEmbed.setThumbnail('http://www.maskedriders.info/Mee6RP/statusPic/Liquidring.png')
              ...
              RiderEmbed.setFooter('Data Card ~' + settings.PlayerClassName(validform), settings.serverpic)
                      ...
                      if ((validform >= 5000) && (validform < 6000)){
                            if ((validform == 5100) || (validform == 5120) || (validform == 5110)){//wiseman, sorcerer, mage
                                    RiderEmbed.addField('Liquid', 'Now!', true)
                                    RiderEmbed.addField('Transform', 'Temporarily transmutates Wizard\'s body into a liquid-like state to make melee attacks useless or to grapple with a foe.', true)
                            } else if (validform == 5000){//wizard
                                    RiderEmbed.addField('Liquid', 'Please.', true)
                                    RiderEmbed.addField('Transform', 'Temporarily transmutates Wizard\'s body into a liquid-like state to make melee attacks useless or to grapple with a foe.', true)
                            } else if (validform == 5140){//beast in magic land
                                    RiderEmbed.addField('Liquid', 'Go!', true)
                                    RiderEmbed.addField('Transform', 'Temporarily transmutates Wizard\'s body into a liquid-like state to make melee attacks useless or to grapple with a foe.', true)
