...
exports.run = (client, msg) => {
	...
	var   playername  = msg.author.username;
        ...
        const RiderEmbed = new discord.RichEmbed()
	      ...
	      RiderEmbed.setThumbnail('http://www.maskedriders.info/Mee6RP/statusPic/Hurricanering.PNG')
              ...
              RiderEmbed.setFooter('Data Card ~' + settings.PlayerClassName(validform), settings.serverpic)
                ...
                if ((validform >= 5000) && (validform < 6000)){
                            if (validform == 5100){//wiseman
                                    ...
                                    connection.query("UPDATE " + settings.dbtable + " SET CLASS_ID_Sub = 5241 WHERE ID = '" + playeruid + "'");
                                    ...
                                    connection.query("UPDATE " + settings.dbtable + " SET CLASS_ID_Sub = 5391 WHERE ID = '" + playeruid + "'");
                                    ...
                                    RiderEmbed.setDescription('Level: 4', true)
                                    connection.query("UPDATE " + settings.dbtable + " SET CLASS_ID_Sub = 5041 WHERE ID = '" + playeruid + "'");
                                    connection.query("UPDATE " + settings.dbtable + " SET Power_Mult = 5 WHERE ID = '" + playeruid + "'");
                            }//if else

                                    if (validform == 5000){
                                        RiderEmbed.addField('Huricane, Dragon', 'Byu-Byu, Byu-Byu-Byu-Byu!', true)
                                        RiderEmbed.addField('Transform', 'Transforms into Huricane Dragon Style.', true)
                                        RiderEmbed.setImage('http://www.maskedriders.info/Mee6RP/statusPic/Hurricanedragon.png')
                                    } else if (validform == 5140){//beast in magic land
                                        RiderEmbed.addField('Huricane', 'Go!', true)
                                        RiderEmbed.addField('Transform', 'Transforms into Huricane Griphon Style.', true)
                                    } else { 
                                        RiderEmbed.addField('Tempest, Phantom', 'Byu-Byu, Byu-Byu-Byu-Byu!', true)
                                        RiderEmbed.addField('Transform', 'Transforms into Tempest Phantom Style.', true)
