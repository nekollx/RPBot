              ...
              RiderEmbed.setFooter('Data Card ~' + settings.PlayerClassName(validform), settings.serverpic)
                      ...
                      if ((validform >= 5000) && (validform < 6000)){
                            if ((validform == 5100) || (validform == 5120) || (validform == 5110)){//wiseman, sorcerer, mage
                                    RiderEmbed.addField('Fragrance', 'Now!', true)
				    RiderEmbed.addField('Summon', 'Creates a pleasent smell on the caster.', true)//driver on, flower, light, bind, copy, drill, smell, dragorise/chemerise, plamonsters, miracle, chichin pui pui, holy, rider rush
                            } else if (validform == 5140){//beast in magic land
                                    RiderEmbed.addField('Perfume', 'Go!', true)
				    RiderEmbed.addField('Summon', 'Creates a pleasent smell on the caster.', true)//driver on, flower, light, bind, copy, drill, smell, dragorise/chemerise, plamonsters, miracle, chichin pui pui, holy, rider rush
                            } else if (validform == 5000){//wizard
                                    RiderEmbed.addField('Perfume', 'Please.', true)
				    RiderEmbed.addField('Summon', 'Creates a pleasent smell on the caster.', true)//driver on, flower, light, bind, copy, drill, smell, dragorise/chemerise, plamonsters, miracle, chichin pui pui, holy, rider rush
