              ...
              RiderEmbed.setFooter('Data Card ~' + settings.PlayerClassName(validform), settings.serverpic)
                ...
                if ((validform >= 5000) && (validform < 6000)){
                            if (validform == 5100){//wiseman
                                    ...
                                    connection.query("UPDATE " + settings.dbtable + " SET CLASS_ID_Sub = 5270 WHERE ID = '" + playeruid + "'");
                                    ...
                                    connection.query("UPDATE " + settings.dbtable + " SET CLASS_ID_Sub = 5360 WHERE ID = '" + playeruid + "'");
                                    connection.query("UPDATE " + settings.dbtable + " SET Power_Mult = 6 WHERE ID = '" + playeruid + "'");
                            } else {//wizard
                                    RiderEmbed.setDescription('Level: 5', true)
                                    connection.query("UPDATE " + settings.dbtable + " SET CLASS_ID_Sub = 5060 WHERE ID = '" + playeruid + "'");
                                    connection.query("UPDATE " + settings.dbtable + " SET Power_Mult = 6 WHERE ID = '" + playeruid + "'");
                            }//if else

                                    if (validform == 5000){//wizard
                                        RiderEmbed.addField('All Dragon', 'Please.', true)
                                        RiderEmbed.addField('Transform', 'Transforms into All Dragon Style.', true)
                                        RiderEmbed.setImage('http://www.maskedriders.info/Mee6RP/statusPic/Wizard_all.png')
                                    } else if (validform == 5140){//beast in magic land
                                        RiderEmbed.addField('All Griphon', 'Go!', true)
                                        RiderEmbed.addField('Transform', 'Transforms into All Griphon Style.', true)
                                    } else {//wiseman
                                        RiderEmbed.addField('All Phantom', 'Now!', true)
                                        RiderEmbed.addField('Transform', 'Transforms into All Phantom Style.', true)
