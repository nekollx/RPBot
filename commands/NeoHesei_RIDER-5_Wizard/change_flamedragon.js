...
exports.run = (client, msg) => {
	...
	var   playername  = msg.author.username;
        ...
        const RiderEmbed = new discord.RichEmbed()
	      ...
	      RiderEmbed.setThumbnail('http://www.maskedriders.info/Mee6RP/statusPic/Flamedragonring.PNG')
              ...
              RiderEmbed.setFooter('Data Card ~' + settings.PlayerClassName(validform), settings.serverpic)
                ...
                if ((validform >= 5000) && (validform < 6000)){
                            if (validform == 5100){//wiseman
                                    ...
                                    connection.query("UPDATE " + settings.dbtable + " SET CLASS_ID_Sub = 5211 WHERE ID = '" + playeruid + "'");
                                    ...
                                    connection.query("UPDATE " + settings.dbtable + " SET CLASS_ID_Sub = 5370 WHERE ID = '" + playeruid + "'");
                                    ...
                                    RiderEmbed.setDescription('Level: 4', true)
                                    connection.query("UPDATE " + settings.dbtable + " SET CLASS_ID_Sub = 5011 WHERE ID = '" + playeruid + "'");
                                    connection.query("UPDATE " + settings.dbtable + " SET Power_Mult = 5 WHERE ID = '" + playeruid + "'");
                            }//if else

                                    if (validform == 5000){
                                        RiderEmbed.addField('Flame, Dragon', 'Bou-Bou, Bou-Bou-Bou!', true)
                                        RiderEmbed.addField('Transform', 'Transforms into Flame Dragon Style.', true)
                                        RiderEmbed.setImage('http://www.maskedriders.info/Mee6RP/statusPic/Flamedragonstyle.png')
                                    } else if (validform == 5140){//beast in magic land
                                        RiderEmbed.addField('Flame Griphon', 'Go!', true)
                                        RiderEmbed.addField('Transform', 'Transforms into Flame Griphon Style.', true)
                                    } else { 
                                        RiderEmbed.addField('Volcano, Phantom', 'Bou-Bou, Bou-Bou-Bou!', true)
                                        RiderEmbed.addField('Transform', 'Transforms into Volcano Phantom Style.', true)
