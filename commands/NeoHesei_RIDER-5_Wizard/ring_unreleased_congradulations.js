...
exports.run = (client, msg) => {
	...
	var   playername  = msg.author.username;
        ...
        const RiderEmbed = new discord.RichEmbed()
	      ...
	      RiderEmbed.setThumbnail('http://www.maskedriders.info/Mee6RP/statusPic/Congratulations_Wizard_Ring.jpg')
              ...
              RiderEmbed.setFooter('Data Card ~' + settings.PlayerClassName(validform), settings.serverpic)
                      ...
                      if ((validform >= 5000) && (validform < 6000)){
                            if ((validform == 5100) || (validform == 5120) || (validform == 5110)){//wiseman, sorcerer, mage
                                    RiderEmbed.addField('Celebrations', 'Now!', true)
				    RiderEmbed.addField('Transform', 'Similar to Dress Up but works on the people around the caster.', true)//small, big, extend, liquid, dress up, excite, clear, special rush, beast mantles, hyper beast
                            } else if (validform == 5140){//beast in magic land
                                    RiderEmbed.addField('Congratulations', 'Go!', true)
				    RiderEmbed.addField('Transform', 'Similar to Dress Up but works on the people around the caster.', true)//small, big, extend, liquid, dress up, excite, clear, special rush, beast mantles, hyper beast
                            } else if (validform == 5000){//wizard
                                    RiderEmbed.addField('Congratulations', 'Please.', true)
				    RiderEmbed.addField('Transform', 'Similar to Dress Up but works on the people around the caster.', true)//small, big, extend, liquid, dress up, excite, clear, special rush, beast mantles, hyper beast
