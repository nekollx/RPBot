...
exports.run = (client, msg) => {
	...
	var   playername  = msg.author.username;
        ...
        const RiderEmbed = new discord.RichEmbed()
	      ...
	      RiderEmbed.setThumbnail('http://www.maskedriders.info/Mee6RP/statusPic/Gravityring.PNG')
              ...
              RiderEmbed.setFooter('Data Card ~' + settings.PlayerClassName(validform), settings.serverpic)
                      ...
                      if ((validform >= 5000) && (validform < 6000)){
                            if ((validform == 5100) || (validform == 5120) || (validform == 5110)){//wiseman, sorcerer, mage
                                    RiderEmbed.addField('Yes!', 'Gravity, Understand?', true)
                                    RiderEmbed.addField('Attack', 'fires a blast of compressed gravity at the enemy to levitate them into the air and then drop them to the ground with a sharp increase in gravity, leaving a sizable crater as a result.', true)                      
                            } else if (validform == 5000){//wizard
                                    RiderEmbed.addField('Chōīne', 'Gravity! Saikō!', true)
                                    RiderEmbed.addField('Attack', 'fires a blast of compressed gravity at the enemy to levitate them into the air and then drop them to the ground with a sharp increase in gravity, leaving a sizable crater as a result.', true)                      
                            } else if (validform == 5140){//beast in magic land
                                    RiderEmbed.addField('Gravity', 'Go!', true)
                                    RiderEmbed.addField('Attack', 'fires a blast of compressed gravity at the enemy to levitate them into the air and then drop them to the ground with a sharp increase in gravity, leaving a sizable crater as a result.', true)                      
