...
//    };//if
    ...
    var slapdesc  = 'Sorry '+ mentionedname + ' I could not set '+ ClassID + ' as your default for '+settings.SeriesName(SeriesID)+' Roleplays.';
            ...
            if (errU) console.log(errU);
                if (resultU.length > 0){
                    var UpdateString  = "UPDATE " + settings.playerforms+" ";
                        UpdateString += "SET ";
                        UpdateString += "Character_Name = '" + ClassID +"' ";
                    ...
                    if ((settings.User_Forms.get(resultU[0].Table_ID).User_ID == mentionedid) && 
                        ...
                        {settings.User_Forms.get(resultU[0].Table_ID).Character_Name = ClassID;}
                        slapdesc  = mentionedname + ' just set '+ ClassID + ' as their default for '+settings.SeriesName(SeriesID)+' Roleplays.';
