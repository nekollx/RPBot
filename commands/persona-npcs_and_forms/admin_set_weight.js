...
exports.run = (client, msg) => {
                ...
                if (isNaN(rankstoadd)){
                    rankstoadd = 1;
                } else if (rankstoadd > settings.WeightCap){
                    rankstoadd = settings.WeightCap;
                } else if (rankstoadd < settings.WeightMin){
                    rankstoadd = settings.WeightMin;
                };//if else

                var slapdesc           = uidname + ' just changed '+ mentionedname + '\'s weight to ' + rankstoadd.toLocaleString() + ' lbs!';
        ...
        mysqlPool.getConnection(function(err, connection) {
            if(err) throw err;
                connection.query("UPDATE " + settings.dbtable + " SET Weight = " + rankstoadd + " WHERE ID = '" + mentionedid + "'");
