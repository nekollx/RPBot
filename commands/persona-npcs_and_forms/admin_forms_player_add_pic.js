            ...
            if (errU) console.log(errU);
                if (resultU.length > 0){
                    var UpdateString  = "UPDATE " + settings.playerforms+" ";
                        UpdateString += "SET ";
                        UpdateString += "Avatar_URL = '" + ClassID +"' ";
                    ...
                    if ((settings.User_Forms.get(resultU[0].Table_ID).User_ID == mentionedid) && 
                        ...
                        {settings.User_Forms.get(resultU[0].Table_ID).Avatar_URL = ClassID;}
                    slapdesc  = mentionedname + ' just set '+ ClassID + ' as their default picture for '+settings.SeriesName(SeriesID)+' Roleplays.';
